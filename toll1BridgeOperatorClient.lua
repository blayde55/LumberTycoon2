...
NPCModule:AddNPC(script.Parent.Merely, function(player, chatSelection)	
	...
	if chatSelection == "ConfirmPurchase" then
		...
		if success then
			local thx = script.Parent.Merely.Dialog.BridgeUp.Dialog.ConfirmPurchase.ItWorked.Thanks
			if downTime.Value <= 0 then
				if checkForEye() then
					downTime.GoSpooky.Value = true
					thx = script.Parent.Merely.Dialog.BridgeUp.Dialog.ConfirmPurchase.ItWorked.Thanks2
			...
			downTime.Value = downTime.Value + 3 * 60
			
			return thx
		else
			return script.Parent.Merely.Dialog.BridgeUp.Dialog.ConfirmPurchase.NotWorked.NoThanks
		end
		
	elseif chatSelection == "Initiate" then
		
		if downTime.Value > 0 then
			return script.Parent.Merely.Dialog.BridgeDown.Dialog
		else
			return script.Parent.Merely.Dialog.BridgeUp.Dialog
		end
	...
	elseif chatSelection == "InquireTime" then
		...
		if minutes > 0 then
			script.Parent.Merely.Dialog.BridgeDown.Dialog.InquireTime.ResponseDialog = minutes.." minutes and "..seconds.." seconds."
		else
			script.Parent.Merely.Dialog.BridgeDown.Dialog.InquireTime.ResponseDialog = seconds.." seconds."
		end
